...
class Higgswidth(PhysicsModel):
    ...
    def setPhysicsOptions(self,physOptions):
        for po in physOptions:
            if po == "GGsmfixed":
                print "Will fix CMS_ww2l2v_GGsm to 1 and float muV, muF"
                self.GGsmfixed = True
            if po == "RVRFfixed":
                print "Will fix muV, muF to 1 and float mu"
                self.RVRFfixed = True
            if po == "GGsmRVRFfixed":
                print "Will fix muV, muF, CMS_ww2l2v_GGsm to 1 and float mu"
    ...
    def doParametersOfInterest(self):
        ...
        self.modelBuilder.doVar("CMS_ww2l2v_GGsm[1.,0.,50.]")
        ...
        self.modelBuilder.out.var("CMS_ww2l2v_GGsm").setVal(1)
        self.modelBuilder.out.var("CMS_widthH_kbkg").setVal(1)

        if self.GGsmfixed:
            self.modelBuilder.out.var("CMS_ww2l2v_GGsm").setConstant(True)
            print "Fixing CMS_ww2l2v_GGsm and R"
            self.modelBuilder.out.var("R").setConstant(True)
            poi = "RV,RF"
        elif self.GGsmRVRFfixed:
            self.modelBuilder.out.var("CMS_ww2l2v_GGsm").setConstant(True)
            print "Fixing CMS_ww2l2v_GGsm and RV, RF"
        ...
        self.modelBuilder.factory_("expr::ggH_s_func(\"@0*@1*@3-sqrt(@0*@1*@2*@3)\",R,CMS_ww2l2v_GGsm,CMS_widthH_kbkg,RF)")
        self.modelBuilder.factory_("expr::ggH_b_func(\"@2-sqrt(@0*@1*@2*@3)\",R,CMS_ww2l2v_GGsm,CMS_widthH_kbkg,RF)")
        self.modelBuilder.factory_("expr::ggH_sbi_func(\"sqrt(@0*@1*@2*@3)\",R,CMS_ww2l2v_GGsm,CMS_widthH_kbkg,RF)")

        self.modelBuilder.factory_("expr::qqH_s_func(\"@0*@1*@2-sqrt(@0*@1*@2)\",R,CMS_ww2l2v_GGsm,RV)")
        self.modelBuilder.factory_("expr::qqH_b_func(\"1-sqrt(@0*@1*@2)\",R,CMS_ww2l2v_GGsm,RV)")
        self.modelBuilder.factory_("expr::qqH_sbi_func(\"sqrt(@0*@1*@2)\",R,CMS_ww2l2v_GGsm,RV)")
